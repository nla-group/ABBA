...
class ABBA(object):
    ...
    def compress(self, time_series):
        ...
        while end < len(time_series):
            ...
            if self.norm == 2:
                err = np.linalg.norm((time_series[start] + (inc/(end-start))*x[0:end-start+1]) - time_series[start:end+1],ord=2)**2
            else:
                err = np.linalg.norm((time_series[start] + (inc/(end-start))*x[0:end-start+1]) - time_series[start:end+1],ord=1)
    ...
    def digitize(self, pieces):
        ...
        # the error I corrected modified  
    ...
    def inverse_compress(self, start, pieces):
        ...
        print("start time series:", time_series)
